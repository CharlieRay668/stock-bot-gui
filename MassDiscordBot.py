...
DEV_BOT_TOKEN = 'NzUzMzg1MjE1MTAzMzM2NTg4.X1laqA.olIbd_XahLPqxLP6uSzTY6Yd6oc'
...
def get_history(individual_trades, name):
    if name == 'SammySnipes' or name == 'MoneyMan' or name == 'Engine Trades' or name == 'Adam B' or name == 'Charlie678':
...
async def deep(ctx, *, description):
    ...
    if status == 200:
        ...
        for index, row in message.iterrows():
            embedVar.add_field(name=index, value='Fill Price: ' + str(row['trade_price']) + ', Quantity: ' + str(row['quantity']) + ', Time: ' + str(row['time']), inline=False)
        await ctx.channel.send(embed=embedVar)
    else:
        await ctx.channel.send(message)
...
#2148,892f3628619511ebb64e69f24171e6df,LULU,OPTION,LULU_031921C400,1,3/19,CALLS,3.7,400.0,3.5,3.7,0.1402,-0.1163,0.0041,0.2753,LULU Mar 19 2021 400 Call,Rich,request-bot🤖,2021-01-28 18:20:44.893143
#1937,12914dc65a6a11ebb64e69f24171e6df,ICLN,OPTION,ICLN_041621C45,1,4/16,CALLS,1.45,45.0,1.35,1.45,0.2337,-0.0216,0.0249,0.0468,ICLN Apr 16 2021 45 Call,Rich,request-bot🤖,2021-01-19 15:21:59.946241
#2013,0d2632285c1011ebb64e69f24171e6df,ICLN,OPTION,ICLN_041621C45,1,4/16,CALLS,1.3,45.0,1.2,1.3,0.2239,-0.0199,0.0263,0.0469,ICLN Apr 16 2021 45 Call,Rich,request-bot🤖,2021-01-21 17:42:37.837403
#2064,1de26fb05f2f11ebb64e69f24171e6df,ICLN,OPTION,ICLN_041621C45,1,4/16,CALLS,1.05,45.0,0.9,1.05,0.1956,-0.0175,0.0267,0.0426,ICLN Apr 16 2021 45 Call,Rich,request-bot🤖,2021-01-25 17:02:35.328173
#2138,c445767c617d11ebb64e69f24171e6df,SPY,OPTION,SPY_031921P360,1,3/19,PUTS,8.32,360.0,8.25,8.32,-0.3004,-0.144,0.0085,0.4911,SPY Mar 19 2021 360 Put,Rich,request-bot🤖,2021-01-28 15:30:36.106943
#2152,7dbcb2c061a911ebb64e69f24171e6df,VXX,OPTION,VXX_031921C20,100,3/19,CALLS,3.2,20.0,3.15,3.2,0.5613,-0.0329,0.0457,0.0285,VXX Mar 19 2021 20 Call,Rich,request-bot🤖,2021-01-28 20:43:37.193754